...
class VGGBtlLadder(chainer.Chain):

    def __init__(self, n_class=11, rloss_weight = 1.0):
        super(VGGBtlLadder, self).__init__()
        ...
        with self.init_scope():
            ...
            self.btl3 = L.Convolution2D(256, 32, 3, pad=1)
            self.btl2 = L.Convolution2D(128, 32, 3, pad=1)
            self.btl1 = L.Convolution2D(64, 32, 3, pad=1)
            self.btlu3 = L.Convolution2D(32, 256, 3, pad=1)
            self.btlu2 = L.Convolution2D(32, 128, 3, pad=1)
            self.btlu1 = L.Convolution2D(32, 64, 3, pad=1)
    ...
    def predict(self, x):
        ...
        h3 = self.btlu3(F.relu(self.btl3(h3)))
        h2 = self.btlu2(F.relu(self.btl2(h2)))
        h1 = self.btlu1(F.relu(self.btl1(h1)))
    ...
    def predict_z(self, x):
        ...
        z3 = F.relu(self.btl3(h3))
        z2 = F.relu(self.btl2(h2))
        z1 = F.relu(self.btl1(h1))

        h3 = self.btlu3(F.relu(z3))
        h2 = self.btlu2(F.relu(z2))
        h1 = self.btlu1(F.relu(z1))

        g2 = F.relu(self.deconv3(h3))
        g1 = F.relu(self.deconv2(F.add(h2 + g2)))
        g0 = F.relu(self.deconv1(F.add(h1 + g1)))

        return h4, g0, [z3, z2, z1]
